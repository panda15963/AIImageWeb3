...
export default function CryptocurrencyAPI() {
  ...
  return (
    <div className="m-8 border border-black rounded-md overflow-hidden shadow-lg">
      <h1 className="text-xl font-bold mb-4 text-center my-4 text-4xl">Stocks & Charts</h1>
      <hr className="border-black" />
      <section className="border border-black rounded-md overflow-hidden shadow-lg m-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-4">
          {Object.entries(CHART_COMPONENTS).map(([currency]) => (
            <button
              onClick={() => setSelectedCurrency(currency)}
              className="bg-white p-4 rounded shadow-xl text-center border border-black hover:bg-gray-100 transition-all cursor-pointer"
              key={currency}
            >
      ...
      <section className="m-4 border border-black rounded-md overflow-hidden shadow-lg">
