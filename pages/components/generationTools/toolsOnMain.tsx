...
export default function ToolsOnMain() {
  ...
  return (
    <div className="m-8 border border-black rounded-md">
      <Row>
          ...
          <div className="my-4">
            ...
            <div className="flex flex-row gap-4 justify-center bg-white shadow-2 p-4">
              {ToolsInformation.map((tool, index) => (
                <div
                  key={index}
                  className="border border-black rounded-md shadow-lg overflow-hidden"
                >
                      ...
                      <div>
                        {user ? (
                          <Link href={tool.tool_link}>
                            <h1 className="uppercase font-bold">{tool.title}</h1>
                          ...
                          <button
                            onClick={() => alert("Sign In first, please!")}
                          >
                            <h1 className="uppercase font-bold">{tool.title}</h1>
                            <h2 className="text-default-500">{tool.description}</h2>
                            <CardBody className="overflow-visible p-2">
